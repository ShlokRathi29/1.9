/**
 * API Client — wired to the Bun + Hono backend
...
// Paths where a 401 means the session is gone → redirect to login
const AUTH_REDIRECT_PATHS = ['/user/me', '/auth/']

async function apiFetch<T>(path: string, options?: RequestInit): Promise<T> {
  const token = typeof window !== 'undefined' ? localStorage.getItem('zapkey_token') : null

  // Merge headers properly — never let ...options overwrite the auth header
  ...
  if (res.status === 401) {
    ...
    // Backend returns { success, error } — read the 'error' field
