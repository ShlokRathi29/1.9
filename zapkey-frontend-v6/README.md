...
| Route | Page |
|---|---|
| `/` | Homepage — hero search + trending cities |
...
- **State**: Zustand store in `lib/store.ts` — replace with real API calls when backend is ready
- **Mock Data**: `lib/mock-data.ts` — swap with API calls to Bun+Hono backend
- **Components**: `components/zapkey/` — domain components; `components/ui/` — shadcn/ui
- **Auth**: Currently mocked — wire to `POST /api/v1/auth/signup` and `/api/v1/auth/login`
- **Token Reveal**: Transactions table uses 1 token per reveal — wire to backend unlock API
